...
const EditTreatment: FC<EditTreatmentProps> = ({
  ...
  const downloadFile = async (): Promise<void> => {
    const schema = anonymizationDetails.input_dataset.dataset_schema.map((col) => {
      if (col.treatment !== Treatment.None) {
        return {
          column: col.key,
          treatment_type: col.treatment,
          treatment_options: col.treatment_options,
        }
      }
    })
    const config = {
      computation_id: anonymization_uuid,
      name: anonymizationDetails.name,
      description: anonymizationDetails.description,
      creation_date: anonymizationDetails.created_at,
      dataset_schema: schema.filter((col) => col != null),
    }
